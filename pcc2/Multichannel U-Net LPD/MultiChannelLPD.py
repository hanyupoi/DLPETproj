...
class MultiChannelLPD(nn.Module):
    ...
    def __init__(
        ...
        self.channel_reducer = nn.Conv2d(256, 1, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x: torch.Tensor, ct_input: torch.Tensor) -> torch.Tensor:
        ...
        if not self.use_3d:
            ...
            if primal_out.size(1) > 1:
                primal_out = self.channel_reducer(primal_out)
        ...
        for i in range(1, self.n_iter):
            ...
            if not self.use_3d:
                ...
                primal_out = self.primal_layers[i](primal_input, ct_2d)
                if primal_out.size(1) > 1:
                    primal_out = self.channel_reducer(primal_out)
                img_step += self._to3DP(primal_out)
            else:
